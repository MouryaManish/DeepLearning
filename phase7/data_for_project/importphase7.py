...
def data_invade():
    """
    combining the saccade columns and fixation columns. the saccade columns are substracted to get
    """
    ...
    d1 = data[:,1]
    d2 = np.zeros((65,6,91,2))
    d3 = np.zeros((65,6,91,3))
    d4 = np.zeros((65,6,91,1))

    for i in range(65):
        for r in range(6):
            if d1[i][r,2][:,:].shape[0]!=91:
                pad = 91 - d1[i][r,2][:,:].shape[0]
                d2[i,r] = np.pad(d1[i][r,2][:,1:3],((0,pad),(0,0)),mode="constant")
                d3[i,r] = np.pad(d1[i][r,3][:,2:5],((0,pad),(0,0)),mode="constant")
    
    index = np.argwhere(np.isnan(d3))
    d3[index[:,0],index[:,1],index[:,2]]= 0
    d4[:,:,:,0] = d2[:,:,:,1]- d2[:,:,:,0]
    d = np.concatenate((d4,d3),axis=3)
    d2 = d.reshape(65,546,4)
    return output,d2
...
def get_data():
   ...
   # print("\nval\n ",y_val.shape)
    ...
    """ 
    testing is uded when we have the model just want to check on that
    _main_train is used in final validation after the average is taken.
     """ 
    if testing: 
