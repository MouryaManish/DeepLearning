...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        score1,score1_catch = affine_relu_forward(X,W1,b1)
        score2, score2_catch = affine_forward(score1,W2,b2)
        scores = score2
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ############################################################################
        loss,dloss =  softmax_loss(score2,y)
        reg_loss = 0.5 * reg * np.sum(W2*W2)
        reg_loss += 0.5 * reg * np.sum(W1*W1)
        loss += reg_loss
        dscore2,dW2,db2 =  affine_backward(dloss,score2_catch)
        dW2 += reg * W2
        dscore1,dW1,db1 = affine_relu_backward(dscore2,score1_catch)
        dW1 += reg * W1
        grads['W1'] = dW1
        grads['W2'] = dW2
        grads['b1'] = db1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=1*55*64, num_classes=2,
        ...
        if self.normalization =='batchnorm':
            ...
            for i in range(1,self.num_layers):
                self.params["gamma%d"%(i)] = np.ones(parameters[i])
                self.params["beta%d"%(i)] = np.zeros(parameters[i])
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
            if self.normalization =="batchnorm":
                layers_output[i],layers_forward_catch[i] = affine_batchnorm_relu_forward(layers_output[i-1],self.params['W%d'%(i)],self.params['b%d'%(i)],self.params["gamma%d"%(i)],self.params["beta%d"%(i)],self.bn_params[i-1]) 
            else:    
                layers_output[i],layers_forward_catch[i] = affine_relu_forward(layers_output[i-1],self.params['W%d'%(i)],self.params['b%d'%(i)])
            
            if self.use_dropout:
                    layers_output[i],layers_forward_catch["drop%d"%(i)] = dropout_forward(layers_output[i],self.dropout_param) 
        ...
        for i in range(self.num_layers-1,0,-1):
            if self.use_dropout:
                dx[i+1] = dropout_backward(dx[i+1],layers_forward_catch["drop%d"%(i)])

            if self.normalization =="batchnorm":
                dx[i],grads['W%d'%(i)],grads['b%d'%(i)],grads["gamma%d"%(i)],grads["beta%d"%(i)] =  affine_batchnorm_relu_backward(dx[i+1],layers_forward_catch[i])
            else:    
